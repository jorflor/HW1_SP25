...
def main():
    ...
    :return: nothing
    """
    N = 10000  # size of array I want
    arr = []  # array for storing the numbers
    mean = 100  # the mean I want
    stdev = 5  # the standard deviation
    ...
    for i in range(N):  # this loop generates the numbers
        arr.append(rnd.normalvariate(mean, stdev))
        # this checks which bin(s) the current number is in.
        if between(arr[i], bin1low, bin1high):
            bin1 += 1
        if between(arr[i], bin2low, bin2high):
            bin2 += 1
        if between(arr[i], bin3low, bin3high):
            bin3 += 1
